...
export function onUpdateTimeLine(this: Player): (self: Timeline) => boolean {
  ...
  return (self: Timeline) => {
    ...
    this.persoChanges.forEach((changes, id) => {
      ...
      // update sets :
      if (currentTime >= (change.next ?? Infinity) || currentTime <= change.curr) {
...
function applyChange({ $el, change, perso, currentTime = null, mediaStatus }: ApplyChange) {
  if (change.className) {
  ...
  if (perso.type == persoTypes.VIDEO && change.media) {
    ...
    if (change.media.action == "play") {
      ...
      $media.change = change.media;
      $video.currentTime = (change.media.offset ?? 0) / 1000;
      $video.play();
    }
    change.media.action == "pause" && $video.pause();
  }
