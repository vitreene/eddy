...
export function MediaEvents() {
	...
	const onChange = (event: TextTime) => {
		if (activeAction) mediaActions?.dispatch({ type: "update", target: activeAction.action, payload: event });
	};
	return (
		<ul className="flex flex-1 flex-wrap items-start border border-amber-200">
			{cues &&
				cues.map((event) => (
