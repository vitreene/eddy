...
export const SpacingMini: React.FC<Props> = ({ value, onChange }) => {
	...
	return (
		<div className="space-y-2">
			<span className="text-[10px] opacity-60">Marge</span>
			<div className="flex items-center gap-2">
				<span className="shrink-0 text-[10px] opacity-60">Int.</span>
				<Slider value={[pad]} onValueChange={([v]) => update("padding", v)} max={40} />
				<span className="w-6 text-right text-[10px]">{pad}</span>
			</div>

			<div className="flex items-center gap-2">
				<span className="shrink-0 text-[10px] opacity-60">Ext.</span>
